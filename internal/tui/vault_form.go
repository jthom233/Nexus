...
// VaultFormCancelMsg is sent when the vault profile form is cancelled (Esc/Ctrl+C).
type VaultFormCancelMsg struct{}
...
type vaultFormModel struct {
	...
	// Field bindings (pointers shared with huh form)
	name         *string
	description  *string
	group        *string
	username     *string
...
func newVaultForm(groups []config.Group, profile *vault.CredentialProfile) vaultFormModel {
	m := vaultFormModel{
		...
		username:     new(string),
		password:     new(string),
		identityFile: new(string),
		passphrase:   new(string),
		domain:       new(string),
		vncPassword:  new(string),
		tags:         new(string),
	}
...
func (m vaultFormModel) Update(msg tea.Msg) (vaultFormModel, tea.Cmd) {
	...
	if m.form.State == huh.StateAborted {
		return m, func() tea.Msg { return VaultFormCancelMsg{} }
	}
